classdef modelgen
    ...
    methods (Static)
        function obj = sensormodel(P_D,lambda_c,range_c)
            ...
            % get FOV dimension:
            [num_rows, num_cols] = size(range_c);
            V = 1.0;            
            if num_rows == 1
                % 1D FOV:
                V = (range_c(2) - range_c(1));
            elseif num_rows == 2
                % 2D FOV:
                V = (range_c(1, 2) - range_c(1, 1))*(range_c(2, 2) - range_c(2, 1));
            else
                V = 1.0;
            end
            
            % compute uniform clutter density:
            obj.pdf_c = 1.0 / V;
            ...
            % compute uniform clutter intensity:
